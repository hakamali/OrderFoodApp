package com.example.lenghia.orderfoodapp.Databases;
...
public class Database extends SQLiteAssetHelper {
    ...
    public List<Order> getCarts(String userPhone) {
        ...
        String[] sqlSelect = {"UserPhone", "ProductName", "ProductId", "Quantity", "Price", "Disscount", "Image"};
    ...
    public void addToCart(Order order) {
        SQLiteDatabase db = getReadableDatabase();
        String query = String.format("INSERT OR REPLACE INTO OrderDetail(UserPhone,ProductId,ProductName,Quantity,Price,Disscount,Image)VALUES('%s','%s','%s','%s','%s','%s','%s');",
                order.getUserPhone(),
                order.getProductId(),
    ...
    public void increaseCart(String userPhone,String foodId) {
        SQLiteDatabase db = getReadableDatabase();
        String query = String.format("UPDATE OrderDetail SET Quantity = Quantity + 1 WHERE UserPhone = '%s' AND ProductId = '%s'",
              userPhone, foodId);
        db.execSQL(query);
    }
